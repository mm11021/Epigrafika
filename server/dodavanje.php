...
function unesi($data, $db){
    ...
    //error_log("Start!", 3, "epigrafika.log\n");
    ...
    //korekcija novembar 2015. if(strlen($oznaka)>=15 || ctype_alnum($oznaka)==false)
    if(strlen($oznaka)>=15 || !preg_match("#^[a-zA-Z0-9\.]+$#", $oznaka)){
        //error_log("Greska prilikom provere oznake!\n", 3, "epigrafika.log");
        return false;
    }

    $natpis = $data->natpis;
    $natpis=trim($natpis);
    //korekcija novembar 2015. if (!preg_match("#^[a-zA-Z0-9 \.\(\)\'\,\.]+$#", $natpis))
    /*
    //korekcija septembar 2016. sklonjena provera sadrzaja natpisa u potpunosti...

    if (!preg_match("#^[a-zA-Z0-9 \.\(\)\'\,\.\[\]\|]+$#", $natpis)){
        //error_log("Greska prilikom provere natpisa!\n", 3, "epigrafika.log");
        return false;
    }
    */
    ...
    if($lokalizovanPodatak==true){
        ...
        //korekcija: 15.novembar 2015. - grad ne mora da bude obavezan
        //$grad = null;
        if($data->grad!==null){
            $grad = trim($data->grad);
        ...
        //$mestoNalaska=null;
        if($data->mestoNalaska!==null){
            $mestoNalaska = trim($data->mestoNalaska);
            if(!preg_match("#^[a-zA-Z ]+$#", $mestoNalaska)){
                //error_log("Greska prilikom provere mesta nalaska!", 3, "epigrafika.log");
                return false;
            }
    ...
    if(!preg_match("#^[a-zA-Z ]+$#", $trenutnaLokacijaZnamenitosti)){
        //error_log("Greska prilikom provere trenutne lokacije znamenitosti!", 3, "epigrafika.log");
        return false;
    }
    ...
    /*
    //korekcija: septembar 2016. sklonjena provera komentara
