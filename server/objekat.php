...
try
{
    ...
    if($method === 'GET')
    {
        ...
        {
            $oznaka = $_GET['oznaka'];
            $query = $db->prepare("select * from Objekat where oznaka=$oznaka" );
            $query->execute();
			...
			$stmt = $db->prepare("select * from (select count(*) as novi, datumKreiranja, '#00bc8c' as boja from Objekat group by datumKreiranja,boja order by datumKreiranja desc limit 7) tmp order by datumKreiranja asc");
			$stmt->execute();
    ...
    else if ($method === 'POST')
    {
        ...
        try {
            ...
            if($rezultat == true){
                ...
                $result->error_message= "Nesto nije u redu sa unosom";
            }
        }catch (Exception $e){
            $result->error_status=true;
    ...
    else if ($method === 'PUT')
    {
        ...
        try {
            ...
            if($rezultat == true){
                ...
                $result->error_status = true;
                $result->error_message= "Nesto nije u redu sa azuriranjem";
    ...
    else if ($method === 'DELETE')
    {
        ...
        if($number_of_url_elements ==1) {
            ...
            try {
                ...
                if($rezultat==true){
                    $result->error_status = false;
                    $result->error_message="Objekat uspešno obrisan!";
                }
                else{
                    $result->error_status = true;
                    $result->error_message="Došlo je do greške, probajte kasnije";
                }
