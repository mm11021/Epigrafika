...
try{
	...
	if($method === 'GET')
    {
        $query="select * from VrstaNatpisa";
    ...
    else if($method === 'PUT')
    {
		...
		if(property_exists($data, "naziv"))
            $naziv = $data->naziv;
		$query = $db->prepare("update VrstaNatpisa set naziv='$naziv' where id=$id");
		$query->execute();
    ...
    else if($method === 'DELETE')
    {
		...
		if(isset($_GET["id"])){
            $id = intval($_GET["id"]);
			$query = $db->prepare("delete from VrstaNatpisa where id=$id");
			$query->execute();
			$broj_redova = $query->rowCount();
			if($broj_redova!=0)
			{
	...
	else if($method==='POST')
	{
		...
		if(property_exists($data, "naziv"))
            $naziv = $data->naziv;
		$query = $db->prepare("insert into VrstaNatpisa(naziv) values('$naziv')");
		$query->execute();  
		$broj_redova = $query->rowCount();
		if($broj_redova==1) $result->error_status = false;
		else $result->error_status = true;
