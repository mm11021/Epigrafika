...
try
{
    ...
    if($method === 'GET')
    {
		...
		if($_GET['type'] === 'jedinsten_username'){
				$ime = $_GET['user']; 
				$query = $db->prepare("select * from Korisnik where korisnickoIme=$ime" );
				$query->execute();
        ...
        if($_GET['type'] === 'zaboravljenaSifra'){
            ...
            if($data == NULL){
                ...
                $q = $db->prepare("UPDATE `Korisnik` SET `sifra` = ? WHERE `korisnickoIme` = ? ");
        ...
        if($_GET['type'] === 'view'){
            ...
            $query = $db->prepare("select * from Korisnik where korisnickoIme='$user'" );
            $query->execute();
            $result->error_status = false;
            $result->data = $query->fetchAll();
            if(count($result->data) == 0)
       ...
       if ($_GET['type'] === 'all'){
           $stmt = $db->prepare("select korisnickoIme, ime, prezime, privilegije, status, institucija, datumRegistrovanja from Korisnik");
           $stmt->execute();
		...
		if ($_GET['type'] === 'chart'){
			$stmt = $db->prepare("select * from (select count(*) as novi, datumRegistrovanja, '#0D52D1' as boja from Korisnik group by datumRegistrovanja,boja order by datumRegistrovanja desc limit 7) tmp order by datumRegistrovanja asc");
			$stmt->execute();
...
else if($method === 'POST')
    {
        ...
        $query = $db->prepare("INSERT INTO `Korisnik` (`korisnickoIme`, `sifra`, `ime`,`prezime`,`email`,`institucija`,`dodatneInformacije`,`privilegije`,`datumRegistrovanja`,`status`) VALUES ('$username','$passwordCrypt', '$ime', '$prezime', '$email', '$institucija', '$info', '$privilegije', '$datum', '$status' )");
		$query->execute();  
    ...
    else if($method === 'DELETE')
		{
			...
			$query = $db->prepare("delete from Korisnik where korisnickoIme=$ime" );
			$query->execute();
			$broj_redova = $query->rowCount();
			if($broj_redova!=0)
			{
...
else if($method === 'PUT')
    {
		...
		if(property_exists($data, "stat"))
                    $stat= $data->stat;
		$query = $db->prepare("UPDATE Korisnik SET privilegije = '$privilegije', status = '$stat', ime='$ime', prezime='$prezime', email='$email',institucija='$institucija'  WHERE korisnickoIme = '$korisnickoIme'");
		$query->execute();
		$br = $query->rowCount();
