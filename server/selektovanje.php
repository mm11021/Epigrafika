...
//            $whereupit = " where ".$osobinaBaza." = :".$osobinaData; Stari
    ...
    public function mesto($data){
        if(property_exists($data, 'mestoNalaska')){
            //analizirati
            if(is_null($data->mestoNalaska) or empty($data->mestoNalaska)){
                /* ako postoji i ima vrednost null ili praznu nisku, scenario je isti kao i da nije postavljeno */
                /* Ako je obelezena opcija za nelokalizovane objekte */
                if(property_exists($data, 'prikaziNelokalizovanePodatke') and $data->prikaziNelokalizovanePodatke==true){
                    /* Mesto moze da bude bilo sta */
                    return "where Mesto.naziv LIKE '%'";
                }
                else{
                    /* Mesto moze da bude bilo sta osim null */
                    return "where Toznaka.mesto <> -1";
                }
            }
            else{
                if(property_exists($data, 'prikaziNelokalizovanePodatke') and $data->prikaziNelokalizovanePodatke==true){
                    /* Mesto da bude ono koje je korisnik navao ili NULL */
                    return "where (Mesto.naziv LIKE :mestoNalaska or Toznaka.mesto=-1)";
                }
                else{
                    /* Treba da bude ono koje je korisnik naveo */
    ...
    public function mesto($data)
    {
        ...
        if(property_exists($data, 'mestoNalaska'))
        {
            if(is_null($data->mestoNalaska) or empty($data->mestoNalaska)){
                ...
                {
                    return "where (Mesto.naziv LIKE :mestoNalaska or Mesto.id=-1)";
                }
            }
        }
...
*/
    ...
    {
       /* $upit = "select Tpleme.naziv AS pleme, Toznaka.oznaka, Toznaka.tekstNatpisa AS natpis, TprovincijaNalaska.naziv AS provincijaNalaska , TgradNalaska.naziv AS gradNalaska, Mesto.naziv AS mestoNalaska , 
       TmodernoImeDrzave.naziv AS modernoImeDrzave , vrstaNatpisa.naziv AS vrstaNatpisa , jezik.naziv AS jezik   , Toznaka.tip, Toznaka.materijal, Toznaka.dimenzije   , Toznaka.komentar   , Ustanova.naziv AS ustanova, 
       TmodernoMesto.naziv AS modernoMesto   , Toznaka.pocetakGodina, Toznaka.pocetakVek, Toznaka.pocetakOdrednica , Toznaka.krajGodina, Toznaka.krajVek, Toznaka.krajOdrednica  , Toznaka.id   
       FROM (select * from Objekat) Toznaka JOIN (select * from Provincija) TprovincijaNalaska ON Toznaka.provincija = TprovincijaNalaska.id  JOIN (select * from Grad) TgradNalaska ON Toznaka.grad = TgradNalaska.id  
       JOIN (select * from ModernaDrzava) TmodernoImeDrzave ON Toznaka.modernaDrzava = TmodernoImeDrzave.id  JOIN (select * from ModernoMesto) TmodernoMesto ON Toznaka.modernoMesto = TmodernoMesto.id  
       (select * from Pleme) Tpleme ON Toznaka.pleme = Tpleme.id  LEFT JOIN Mesto ON Toznaka.mesto = Mesto.id  LEFT JOIN vrstaNatpisa ON Toznaka.vrstaNatpisa = vrstaNatpisa.id  
       LEFT JOIN jezik ON Toznaka.jezik = jezik.id  LEFT JOIN ustanova on Toznaka.ustanova = Ustanova.id where Toznaka.id = :idObjekta ";*/
        $upit = "select Tpleme.naziv AS plemeNaziv, Toznaka.*, Toznaka.tekstNatpisa AS natpis, TprovincijaNalaska.naziv AS provincijaNalaska , TgradNalaska.naziv AS gradNalaska, Mesto.naziv AS mestoNalaska , 
        TmodernoImeDrzave.naziv AS modernoImeDrzave , VrstaNatpisa.naziv AS vrstaNatpisa , Jezik.naziv AS jezik  , Ustanova.naziv AS ustanova, TmodernoMesto.naziv AS modernoMesto
        FROM (select * from Objekat) Toznaka JOIN (select * from Provincija) TprovincijaNalaska ON Toznaka.provincija = TprovincijaNalaska.id  JOIN (select * from Grad) TgradNalaska ON Toznaka.grad = TgradNalaska.id  
        JOIN (select * from ModernaDrzava) TmodernoImeDrzave ON Toznaka.modernaDrzava = TmodernoImeDrzave.id  JOIN (select * from ModernoMesto) TmodernoMesto ON Toznaka.modernoMesto = TmodernoMesto.id  
        JOIN (select * from Pleme) Tpleme ON Toznaka.pleme = Tpleme.id  LEFT JOIN Mesto ON Toznaka.mesto = Mesto.id  LEFT JOIN VrstaNatpisa ON Toznaka.vrstaNatpisa = VrstaNatpisa.id
        LEFT JOIN Jezik ON Toznaka.jezik = Jezik.id  LEFT JOIN Ustanova on Toznaka.ustanova = Ustanova.id where Toznaka.id = :idObjekta ";
